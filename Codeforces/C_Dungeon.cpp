...
ll solve(){
    ...
    vector<ll>mos(m),gai(m);
    vector<pair<ll,ll>>com(m);
    loop(i,m) cin >> mos[i];
    loop(i,m) cin >> gai[i];
    loop(i,m) com[i] = {mos[i],gai[i]};
    sort(all(com)); 
    vector<pair<ll,ll>> temp1;
    vector<ll> zeros;
    
    for(auto &i: com){
        if(i.second > 0) temp1.push_back(i);
        else zeros.push_back(i.first);
    }

    multiset<ll> ms(all(swo));
    ll ans = 0;

   
    for(auto &i: temp1){
        ll b = i.first, c = i.second;
    ...
    sort(all(zeros));
    vector<ll> temp2; 
    for(ll x: ms) temp2.push_back(x); 
    
    ll i = 0, j = 0;
    ll S = (ll)temp2.size(), Z = (ll)zeros.size();
    while(i < S && j < Z){
        if(temp2[i] >= zeros[j]){
            ++ans;
            ++i; ++j;
