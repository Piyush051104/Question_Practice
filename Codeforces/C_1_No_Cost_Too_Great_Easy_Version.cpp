...
vector<int> factorize_with_spf(int n, const vector<int>& spf) {
    vector<int> res;
    if (n <= 1) return res;
    while (n != 1) {
        int p = spf[n];
...
ll solve(auto spf){
    ...
    vector<bool>flag(2*1e5 + 1,false);
    for(ll i = 0 ;i < v1.size();i++){
        auto f = factorize_with_spf(v1[i], spf);
    ...
    for(ll i = 0 ;i < v1.size();i++){
        auto f = factorize_with_spf(v1[i] + 1, spf);
