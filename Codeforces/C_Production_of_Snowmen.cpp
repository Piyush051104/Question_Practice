...
vector<ll> build(vector<ll>& b,vector<ll>& c, ll n) {
    ...
    for (ll r = 0; r < n; ++r) {
        ...
        vector<ll> ps(two + 1, 0);
        for (ll p = 0; p < two; p++) {
            ll val = (b[p] < c[p + r]) ? 1 : 0;
            ps[p + 1] = ps[p] + val;
        }
        for (ll start = 0; start < n; ++start) {
            if (ps[start + n] - ps[start] == n) counts[start] += 1;
        }
    }
    return counts;
}
ll solve(){
    ...
    ll ans = 0;
    ll si = 3*n;
    vector<ll> a(si), b(si), c(si);
    for (ll i = 0; i < si; ++i) {
        a[i] = v1[i % n];
        b[i] = v2[i % n];
        c[i] = v3[i % n];
    }
    vector<ll> temp = build(b, c, n);
    ll two = 2 * n;
    for (ll i = 0; i < n; ++i) {
        vector<ll> ps(two + 1, 0);
        for (ll p = 0; p < two; ++p) {
