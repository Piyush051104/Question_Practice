...
ll solve(ll in,vector<pair<pair<ll,ll>,ll>>&v,vector<ll>&dp,ll n,vector<ll>&temp){
    if(in >= n) return 0;
    ...
    ll index = upper_bound(all(temp),v[in].first.second) - temp.begin();

    ans1 = v[in].second + solve(index,v,dp,n,temp);
    ans2 = solve(in + 1,v,dp,n,temp);

    return dp[in] = max(ans1,ans2);
