...
void dfs(ll node,ll par,vector<vector<ll>>&graph,vector<ll>&dis){

    for(auto &child:graph[node]){
        if(par != child){
            dis[child] = dis[node] + 1;
            dfs(child,node,graph,dis);
        }
    }
    return;
}
ll solve(){
    ...
    vector<ll>andis(n + 1);
    andis[node] = 0;
    dfs(node,-1,graph,andis);
    maxi = 0;
    for(ll i = 0 ;i < k;i++){
        maxi = max(maxi,andis[v[i]]);
    }
    return (maxi + 1)/2;
