...
ll find(ll in, ll k, ll n, vector<ll>& v, vector<vector<ll>>& dp) {
    ...
    res = find(in + 1, k, n, v, dp);

    ll mini = LLONG_MAX;
    ll sum = 0;
    for(ll i = 0 ;i + in < n;i++){
        mini = min(mini,v[in + i]);
        sum += v[in + i];
        ...
        ll ans = find(in + i + 1,k - i,n,v,dp);

        if(ans == INF) continue;
        res = min(res,diff + ans);
    }
    return dp[in][k] = res;
}
...
ll solve(ll &cnt){
    ...
    if(n == 1) return v[0];
    if(n == 2){
        if(k == 0) return v[0] + v[1];
        else return 2 * min(v[0],v[1]);
    }
    vector<vector<ll>>dp(n + 1,vector<ll>(k + 1,-1));

    ll ans = find(0,k,n,v,dp);
