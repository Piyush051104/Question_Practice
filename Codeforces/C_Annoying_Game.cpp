...
ll solve(){
    ...
    if(k % 2 == 0){
        ...
        ll ans = max(a[0] + b[0],a[0] + b[0] + suff[1]);
        for(ll i = 1 ;i < n - 1;i++){
            ll val1 = max(pref[i - 1],(ll)0);
            ll val2 = max(suff[i + 1],(ll)0);
            ll val = max(a[i] + b[i] ,a[i] + b[i] + val1 + val2);
            ans = max(val,ans);
        }
        ll a1 = max(pref[n - 2],(ll)0);
        ll a2 = max(a[n - 1] + b[n - 1],a[n - 1] + b[n - 1] + a1);
        ans = max(ans,a2);
        return ans;
    }
