...
ll find(ll node,ll par,ll status,ll k,vector<ll>&v,vector<vector<ll>>&graph,vector<vector<ll>>&dp){
    if(dp[node][status] != -1) return dp[node][status];
    ll ans1 = 0;

    if(status == 1) ans1 = v[node];
    for(auto &child:graph[node]){
        ll ans = 0;
        if(child != par){
            if(status == 0){
                ans = max(ans,find(child,node,1,k,v,graph,dp));
                ans = max(ans,find(child,node,0,k,v,graph,dp));
        
            }

            else{
                
                ans = max(ans,-2*k + find(child,node,1,k,v,graph,dp));
                ans = max(ans,find(child,node,0,k,v,graph,dp));
            }
        }
        ans1 += ans;
...
ll solve(){
    ...
    vector<vector<ll>>dp(n + 2,vector<ll>(2,-1));

    ll ans = find(1,-1,0,k,v,graph,dp);
    ans = max(ans,find(1,-1,1,k,v,graph,dp));
    return ans;
}
int main(){
