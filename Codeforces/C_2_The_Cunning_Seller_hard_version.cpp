...
ll solve(vector<ll>&power,map<ll,ll>&mp){
    ll n,k; cin >> n >> k;
    if(n == k) return n * 3;
    ll steps = k;
    ll ans = 0;
    ...
    map<ll,ll>temp;
    while(rem > 0 && k > 0){
        ll in  = upper_bound(all(power),rem) - power.begin();
        in--;
        rem -=power[in];
    ...
    // }
    ll extra = steps - minstep;
    while(extra >= 2){
        ll firstnum = temp.begin() -> first;
        ll value =  (-1) * (firstnum);
        //cout << value << " ";
        if(value == 1) break;
        ll count = temp.begin() -> second;
        ll need = count * 2;
        if(extra >= need){
            temp.erase(firstnum);
            temp[(value/3) * (-1)] += (count) * 3;
            extra -= need;
        }
        else{
            ll qu = extra/2;
...
int main(){
    ...
    mp[1] = 3;
    for(ll i = 1 ;i < 22 ;i++){
        mp[power[i]] = power[i + 1] + i * power[i - 1];
    }
    ll t; cin >> t;
    while(t--){
        ll ans = solve(power,mp);
        cout << ans << endl;
    }
    return 0;
}