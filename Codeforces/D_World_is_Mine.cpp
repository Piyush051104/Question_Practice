...
ll findans(ll in,ll cnt,vector<ll>&temp1,vector<ll>&temp2,vector<vector<ll>>&dp){
    if(in >= temp1.size()) return 0;
    ...
    if(eat >= 0){
        ans1 = 1 + findans(in + 1,eat,temp1,temp2,dp);
        ans2 = findans(in + 1,cnt + 1,temp1,temp2,dp);
    }
    else{
        ans2 = findans(in + 1,cnt + 1,temp1,temp2,dp);
    }

    return dp[in][cnt] = max(ans1,ans2);
}
ll solve(){
    ll n;cin >> n;
    vector<ll>v(n);
    ...
    ll ans = findans(0,0,temp1,temp2,dp);
    return temp1.size() - ans;
