...
ll find(ll in,ll n,string &s1,string &s2,vector<ll>&dp){
    if(in >= n) return 0;
    ...
    if(s1[in] != s2[in]){
        ll val = find(in + 1,n,s1,s2,dp);
        if(val != LLONG_MAX) ans = min(ans,1 + val);
    }
    if(s1[in] == s2[in]){
        ans = min(ans,find(in + 1,n,s1,s2,dp));
    }

    if(in + 1 <= n - 1){
        if(s1[in] == s1[in + 1]){
            
            ll val = find(in + 2,n,s1,s2,dp);

            if(val != LLONG_MAX){
                if(s2[in] == s2[in + 1]){
                    ans = min(ans,val);
                }
                else ans = min(ans,1 + val);
            }
        }
        else{
            ll val = find(in + 2,n,s1,s2,dp);
