...
void dfs(ll node,vector<ll>&parent,vector<bool>&vis,vector<ll>&order){
    vis[node] = true;
    order.push_back(node);
    ll par = parent[node];
    if(vis[par] == false){
        dfs(par,parent,vis,order);
    }
}
void solve(){
    ll n; cin >>  n;
    ...
    for(ll i = 1 ;i <= n;i++){
        if(vis[i] == false){
            vector<ll>order;
            dfs(i,parent,vis,order);
            ll mini = LLONG_MAX,ind = 0;
            for(ll j = 0; j < order.size();j++){
                if(mini > value[order[j]]){
                    mini = value[order[j]];
                    ind = j;
                }
            }
            for(ll j = ind + 1 ; j < order.size() ;j++){
