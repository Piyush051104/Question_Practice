...
vector<ll>dis(1000 + 1,1e18);
ll findans(ll in,ll use,vector<ll>&v1,vector<ll>&v2,vector<vector<ll>>&dp,ll n,ll k){
    if(in >= n) return 0;
    if(use > k) return 0;
    if(dp[in][use] != -1) return dp[in][use];

    ll need  = dis[v1[in]];

    ll ans1 = 0 , ans2 = 0;
    if(need + use <= k){
        ans1 += v2[in] + findans(in + 1,use + need,v1,v2,dp,n,k);
    }

    ans2 = findans(in + 1,use , v1,v2,dp,n,k);

    return dp[in][use] = max(ans1,ans2);
...
ll solve(){
    ...
    vector<vector<ll>>dp(n + 1, vector<ll>(12 * n,-1));

    return findans(0,0,v1,v2,dp,n,k);
}
int main(){
    ...
    dis[1] = 0;
    bool flag = true;
    while(!qu.empty() && flag == true){
        ll top = qu.front();
        qu.pop();
        if(top >= 10000) continue;
        for(ll i = 1 ; i <= top;i++){
            ll num = top + (top/i);
            
            if(num <= 1000  && dis[num] > dis[top] + 1){
                qu.push(num);
                dis[num] = dis[top] + 1;
            }
        }
