...
bool solve(ll &cnt){
    ...
    if(n < 3) return true;
    if(n == 3){
        if(s[0] == '1' && s[1] == '0' && s[2] == '1') return false;
        else return true;
    }
    if(s[0] == '1' && s[1] == '0' && s[2] == '1' && s[3] == '1') return false;
    if(s[n - 4] == '1' && s[n -3] == '1' && s[n - 2] == '0' && s[n -1] == '1') return false;

    for(ll i = 0 ;i < n - 4 ;i++){
        if(s[i] == '1' && s[i + 1] == '1' && s[i + 2] == '0' && s[i + 3] == '1' && s[i + 4] == '1') return false;
    }
    ll count = 0;
    for(ll i = 0 ;i < n ;i++){
        if(count % 2 == 0 && s[i] == '0'){
            if(i != 0 && s[i - 1 ] == '1' && i +  1  <= n - 1 && s[i + 1] == '1' &&  i + 2  <= n - 1 && s[i + 2] == '1' && s[0] == '1') return false;
            if(i != 0 && i + 1 == n -1 && s[i + 1] == '1' && s[i - 1] != '0' && s[0] == '1') return false;
        }
        if(s[i] == '0') count++;
