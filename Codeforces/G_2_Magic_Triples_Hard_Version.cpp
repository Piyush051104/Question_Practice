...
ll solve(){
    ...
    for(ll i = 0 ;i < n;i++){
        if (v[i] > 1e6) {
                for (ll b = 2; b * v[i] <= 1e9; ++b) {
                    if (v[i] % b == 0 && mp1.find(v[i] / b) != mp1.end() && mp1.find(v[i] * b) != mp1.end()) {
                        ans += (ll)(mp1[v[i] / b]) * (mp1[v[i] * b]) * mp1[v[i]];
                    }
                }
        }
        else{
            ll val = v[i];
            ans += (mp1[v[i]] - 1) * (mp1[v[i]] - 2);
            for(ll j = 2 ;j * j * val <= 1000000;j++){
                ll val1 = val * j;
                ll val2 = val1 * j;
                if(mp1[val1] > 0 && mp1[val2] > 0){
                    ans = ans + (mp1[val1] * mp1[val2]);
                }
            }
        }
