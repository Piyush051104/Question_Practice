...
ll findans(ll in,ll state,ll n,vector<ll>&v,vector<vector<ll>>&dp,vector<string>&str,vector<string>&rev){
    if(in >= v.size()) return 0;
    if(dp[in][state] != -1) return dp[in][state];

    string temp = state ? rev[in - 1] : str[in - 1];
    ll ans1 = LLONG_MAX,ans2 = LLONG_MAX;
    if(str[in] >= temp) ans1 = findans(in + 1,0,n,v,dp,str,rev);
    if(rev[in] >= temp){
        ll val = findans(in + 1,1,n,v,dp,str,rev);
        if(val != LLONG_MAX) ans2 = v[in] + val;
        
    }

    return dp[in][state] = min(ans1,ans2);
...
ll solve(){
    ...
    for(int i=0;i<n;i++) reverse(all(rev[i]));
    vector<vector<ll>>dp(n + 1,vector<ll>(2,-1));

    ll ans1 = findans(1,0,n,v,dp,str,rev);

    ll ans2 = v[0];
    ll ans3 = findans(1,1,n,v,dp,str,rev);

    if(ans3 != LLONG_MAX) ans3 += ans2;
