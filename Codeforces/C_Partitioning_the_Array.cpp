...
ll solve(){
    ...
    for(ll i = 0; i < res.size();i++){
        ...
        ll tempgcd = abs(v[0] - v[res[i]]);
        bool flag = true;
        for(ll j = 0 ; j < n;j++){
            if(j + res[i] <= n - 1 && v[j] != v[j + res[i]]) flag = false;
            if(j + res[i] <= n - 1) tempgcd = __gcd(tempgcd,abs(v[j] - v[j + res[i]]));
        }
        if(tempgcd > 1 || flag == true) ans++;
        //cout << ans << " ";
    }
    return ans;
}
