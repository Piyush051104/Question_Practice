...
ll solve(){
    ...
    vector<pll> vals;
    vals.reserve((size_t)n * 30);
    for(ll i=0;i<n;i++){
        ll cur = v[i];
        while(cur <= LIMIT){
            vals.emplace_back(cur, i);
            if(cur > (LIMIT >> 1)) break;
            cur = cur * 2;
        }
    }
    sort(all(vals), [](const pll &a, const pll &b){
    ...
    for(int r = 0; r < (int)vals.size(); ++r){
        int idx = (int)vals[r].second;
        if(cnt[idx] == 0) ++covered;
        ++cnt[idx];
        while(covered == n && l <= r){
            ll curRange = vals[r].first - vals[l].first;
            if(curRange < best) best = curRange;
            int idxL = (int)vals[l].second;
            --cnt[idxL];
            if(cnt[idxL] == 0) --covered;
            ++l;
        }
    }
    return best;
