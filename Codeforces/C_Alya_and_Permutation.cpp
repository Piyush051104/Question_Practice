...
void solve(){
    ...
    if(n == 7){
        ...
        cout << temp << endl;
        for(ll i = 1 ;i <= temp;i++){
            if(i != power[in - 1] && i != power[in - 1] - 1 && i!= 1 && i != 3 && i != power[in - 1] - 2 && i != temp){
                cout << i << " ";
            }
        }
        cout << 3 << " " << 1 << " " << power [in - 1] - 2 << " " << power[in - 1] - 1 << " " << power[in - 1] << " " << temp  << endl;
    }
    else{
        ll in = upper_bound(all(power),n) - power.begin();
        cout << power[in] - 1 << endl;
        for(ll i = 1 ;i <= n;i++){
            if(i != power[in - 1] && i != power[in - 1] - 1 && i!= 1 && i != 3 && i != power[in - 1] - 2){
                cout << i << " ";
            }
        }
        cout << 3 << " " << 1 << " " << power [in - 1] - 2 << " " << power[in - 1] - 1 << " " << power[in - 1] << endl;
    }
    return;
}
int main(){
