...
ll solve(){
    ...
    if(n == 2){
        ...
        ll check = abs(v[0] - v[1]) * 2;
        return max(sum,check);
    }
    if(v[0]== maxi || v[n  - 1] == maxi) return n * maxi;
    ...
    ll sum3 = v[0] + b * 2;
    ll sum4 = v[2] + a * 2;
    ll sum5 = abs(v[0] - v[2]) * 3;
    ll sum6 = v[0] + v[1] + v[2];
    ll sum7 = 3 * v[0];
    ll sum8 = 3 * v[2];
    //cout << sum1 << " " << sum2 << " " << sum3 << " " << sum4 <<" " << sum5 << " " << sum << endl;
    return max({sum1,sum2,sum3,sum4,sum5,sum6,sum7,sum8});
}
int main(){
