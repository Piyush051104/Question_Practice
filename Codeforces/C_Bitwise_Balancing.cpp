...
ll solve(){
    ll b,c,d;cin >> b >> c >> d;
    string n1=  bitset<64>(b).to_string();
    string n2=  bitset<64>(c).to_string();
    string n3=  bitset<64>(d).to_string();
    string ans = "";
    for(ll i = 0 ;i < 64;i++){
        if(n1[i] == '0' && n2[i] == '1' && n3[i] == '1') return -1;
        if(n1[i] == '1' && n2[i] == '0' && n3[i] == '0') return -1;
        else{
            if(n1[i] == '0'){
                if(n3[i] == '1') ans += '1';
                else ans += '0';
    ...
    unsigned long long val = stoull(ans, nullptr, 2);
    return val;
}
int main(){
