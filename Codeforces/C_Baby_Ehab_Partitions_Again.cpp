...
bool find(ll in,ll cursum,ll sum,vector<ll>&v,vector<vector<ll>>&dp){
    if(in >= v.size()){
        return 2*cursum == sum;
    }

    if(dp[in][cursum] != -1) return dp[in][cursum];

    if(cursum * 2 == sum) return dp[in][cursum] = true;

    bool ans1 = false,ans2 = false;
    ans1 = find(in + 1,cursum + v[in],sum,v,dp);

    ans2 =  find(in + 1,cursum,sum,v,dp);

    return dp[in][cursum] = ans1 || ans2;
}
void solve(){
    ll n;cin >> n;
    ll sum = 0;
    ...
    ll curr = 0;
    bool flag = find(0,curr,sum,v,dp);
