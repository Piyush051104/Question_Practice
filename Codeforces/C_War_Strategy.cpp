...
bool flag(ll n, ll m, ll k, ll len) {
    ...
    ll amin = max((ll)0,s - R);
    ll amax = min(L, s);
    if (amin > amax) return false;

    auto max_side = [&](ll a) -> ll {
        ll b = s - a;
        return max(a, b);
    };

    
    ll a1 = check(s / 2, amin, amax);
    ll a2 = check((s + 1) / 2, amin, amax);
    ll best = min(max_side(a1), max_side(a2));
