...
void solve(){
    ...
    for(int i = 1;i <= n; i++) index[i].push_back(n + 1);
    vector<int>ind(n + 1,n + 1);
    for(int i = 1;i <= n ;i++){
        if(index[i].size() == 1) continue;
        int mini = index[i][0];
        for(int j = 0;j < index[i].size() - 1 ; j++){
            mini = max(mini , abs(index[i][j+1] - index[i][j]) );
        }
        ind[i] = mini;
    }

    vector<int>segment(n + 2,n + 1);
    for(int i = 1; i<= n; i++){
        int seg = ind [i];
        int num = i;
        segment[seg] = min (segment[seg] , num);
    }
    int ans = INT_MAX;
    for(int i = 1;i <= n; i++){
        if(segment[i] == n+1 && ans == INT_MAX) cout << -1 <<" ";
        else {
            ans = min(ans,segment[i]);
            cout << ans <<" ";
        }
    }
    cout << endl;
