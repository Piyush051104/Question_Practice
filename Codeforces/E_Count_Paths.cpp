...
map<ll,ll> dfs(ll node,ll par,ll &ans,vector<ll>&col,vector<vector<ll>>&graph){
    map<ll,ll> mp;

    for(auto &child:graph[node]){
        if(child == par) continue;

        map<ll,ll> re = dfs(child,node,ans,col,graph);

        auto it = re.find(col[node]);
        if(it != re.end()){
            ans += it->second;
            re.erase(it);
        }

        if(re.size() > mp.size()) swap(re, mp);
