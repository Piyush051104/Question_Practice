...
void dfs(ll node,ll parent,ll level,vector<ll>&lvl,vector<vector<ll>>&graph,vector<ll>&par){
    lvl[node] = level;
    par[node] = parent;
    for(auto child:graph[node]){
        if(child != parent){
            dfs(child,node,level + 1,lvl,graph,par);
        }
    }
...
ll solve(){
    ...
    vector<ll>lvl(n + 1);
    vector<ll>par(n + 1);
    dfs(v,0,0,lvl,graph,par);

    ll maxdepth = 0;
    for(ll i = 1 ;i <= n;i++){
        maxdepth = max(maxdepth,lvl[i]);
    }
    return 2 * (n  - 1) - maxdepth + sz/2;
}
int main(){
