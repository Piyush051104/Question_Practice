...
ll solve(){
    ...
    while(!pq.empty()){
        auto top = pq.top();
        pq.pop();
        if(top.second.first == x2 && top.second.second == y2) return top.first;
        ll dis = top.first;

        vector<ll>r = {0,0,1,-1,1,1,-1,-1};
        vector<ll>c = {-1,1,0,0,1,-1,1,-1};
        for(ll i = 0 ;i < 8;i++){
            ll nr = r[i] + top.second.first;
            ll nc = c[i] + top.second.second;

            if(nr >=0 && nc >= 0 && st.find({nr,nc}) != st.end() && mp.find({nr,nc}) == mp.end()){

                pq.push({dis + 1,{nr,nc}});
                mp[{nr,nc}] = dis + 1;
                if(nr == x2 && nc == y2) return dis + 1;
            }
        }
    }
    return -1;
