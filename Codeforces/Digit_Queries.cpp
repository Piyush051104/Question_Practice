...
ll solve(){
    ...
    lenofdigit = st - 1;
    ll startnum = pow(10,lenofdigit - 1);
    preval++;
    ll rem = k - preval + 1;
    ll digittoadd = rem/lenofdigit;
    ll expnum = startnum + digittoadd - 1;
    if(rem % lenofdigit == 0){
        return expnum % 10;
    }
    else{
        ...
        vector<ll>temp;
        while(nextnum > 0){
            temp.push_back(nextnum % 10);
            nextnum /= 10;
        }
        reverse(all(temp));
        ll remaid = rem %  lenofdigit;
        return temp[remaid - 1];
    }
