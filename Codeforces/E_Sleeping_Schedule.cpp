...
ll findans(ll in,ll time,vector<ll>&v,ll h,ll l,ll r,vector<vector<ll>>&dp){
    ...
    ans = (l <= t1 && r >= t1) ? 1 + findans(in + 1,t1,v,h,l,r,dp) : findans(in + 1,t1,v,h,l,r,dp);
    ans = (l <= t2 && r >= t2) ? max(ans,1 + findans(in + 1,t2,v,h,l,r,dp)) : max(ans,findans(in + 1,t2,v,h,l,r,dp));
    // if((time + v[in] - 1) % h >= l && (time + v[in] - 1) % h <= r){
    //     ans = 1 + findans(in + 1,(time + v[in] - 1) % h ,v,h,l,r,dp);
    // } 
    // if((time + v[in]) % h >= l && (time + v[in]) % h <= r){
    //     ans = max(ans,1 + findans(in + 1,(time + v[in] ) % h ,v,h,l,r,dp));
    // }
    // if((time + v[in] - 1) % h < l || (time + v[in] - 1) % h > r){
    //     ans = max(ans, findans(in + 1,(time + v[in] - 1) % h ,v,h,l,r,dp));
    // } 
    // if((time + v[in]) % h < l || (time + v[in]) % h > r){
    //     ans = max(ans,findans(in + 1,(time + v[in] ) % h ,v,h,l,r,dp));
    // }

    return dp[in][time] = ans;
