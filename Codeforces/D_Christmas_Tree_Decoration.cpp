...
ll solve(vector<ll> &fact,vector<ll>&invfact){
    ll n;cin >> n;
    ...
    ll a0 = a[0];
    vector<ll> persons;
    for (ll i = 1; i <= n; ++i) persons.push_back(a[i]);
    ...
    ll ways = C(cntL, x) * C(cntH, r - x) % MOD;
...
int main(){
    ...
    for (ll i = MAXN-2; i >= 0; --i) invfact[i] = invfact[i+1] * (i+1) % MOD;

    
    ll t; cin >> t;
    while(t--){
        ll ans = solve(fact,invfact);
        cout << ans << endl;
    }
    return 0;
}