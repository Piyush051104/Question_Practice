...
ll solve(){
    ...
    vector<vector<ll>>dis(n + 1,vector<ll>(1001 + 1,LLONG_MAX));
    ...
    while(!pq.empty()){
        ...
        for(auto &child:graph[top.second.second]){
            ll newbike = min(top.second.first,s[child.first]);

            if(top.first + child.second * top.second.first < dis[child.first][top.second.first]){
                dis[child.first][top.second.first] = top.first + child.second * top.second.first;
                pq.push({top.first + child.second *  top.second.first,{newbike,child.first}});
            }
        }
    }
    ll ans = LLONG_MAX;
