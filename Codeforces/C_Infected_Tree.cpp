...
ll dfs(ll node,ll parent,vector<vector<ll>>&graph,vector<ll>&num){
    ll si = 1;

    for(auto child:graph[node]){
        if(child != parent){
            si += dfs(child,node,graph,num);
        }
    }
...
ll find(ll node,ll parent,vector<ll>&dp,vector<vector<ll>>&graph,vector<ll>&num){
    ...
    if(childs.size() == 1){
        return dp[node] = num[childs[0]] - 1;
    }
    for(ll i = 0 ;i < childs.size() ;i++){
        ll val = num[childs[i]] - 1;
        for(ll j = 0 ; j < childs.size();j++){
            if(j != i ) ans = max(ans ,val + find(childs[j],node,dp,graph,num));
        }
    }
