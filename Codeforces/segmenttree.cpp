...
class SegmentTree{
    ...
    void pointupdate(int idx,int val,int l,int r,int node){
        push(node,l,r);
    ...
    void rangeupdate(int node,int lq,int rq,int l,int r,int val){
        push(node,l,r);
        if(lq > r  || l > rq) return ;
        if(l >= lq && r <= rq){
            lazy[node] += val;
...
int main(){
    ...
    for(int i = 0 ;i < q;i++){
        int ty;cin >> ty;
        if(ty==1){
            int in,num;cin >> in >> num;
            in--;
            ST.pointupdate(in,num,0,n-1,1);
        }
        if(ty == 2){
            int lq,rq;cin >> lq >> rq;
            lq--,rq--;
            int ans = ST.query(lq,rq,0,n - 1,1);
            cout << ans << endl;
        }
        if(ty == 3){
            int lq,rq,val;cin >> lq >> rq >> val ;
            lq--,rq--;
            ST.rangeupdate(1,lq,rq,0,n - 1,val);
        }
    }
}