...
void find(ll in,string &temp,vector<string>&store,ll k,string &curr){
    if(curr.length() == k){
        store.push_back(curr);
        return;
    }
    if(in == temp.size()) return;
    
    curr+=temp[in];
    find(in + 1,temp,store,k,curr);
    curr.pop_back();
    find(in + 1,temp,store,k,curr);
...
ll solve(){
    ...
    vector<string>store;
    string curr = "";
    find(0,temp,store,k,curr);
    ll ans = 0;
    for(auto& i:store){
        set<char>te;
        for(auto &j:i) te.insert(j);
        ll cnt = 0;
        ll in = 0;
        for(ll k = 0 ;k < n;k++){
            if(a[k] == b[k] || te.count(a[k])){
                in++;   
            }
            else{
                cnt += (in * (in + 1))/2;
