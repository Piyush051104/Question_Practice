...
ll find(ll in,ll prev,ll n,vector<pair<ll,ll>>&v,vector<vector<ll>>&dp){
    if(in >= n) return 0;

    if(dp[in][prev + neu]!= -1) return dp[in][prev + neu];
    ll ans = LLONG_MAX;
    for(ll i = -1 ;i <= 1;i++){
        if(in == 0){
            ans = min(ans,1 + find(in + 1,i,n,v,dp));
        }
        else{
            if(v[in - 1].first + prev == v[in].first + i){
                ans = min(ans,find(in + 1,i,n,v,dp));
            }
            else{
                ans = min(ans,1 + find(in + 1,i,n,v,dp));
            }
...
void solve(){
    ...
    n = cnt.size();
    vector<vector<ll>>dp1(n + 1,vector<ll>(6,-1));
    
    
    ll mini = find(0,0,n,cnt,dp1);
