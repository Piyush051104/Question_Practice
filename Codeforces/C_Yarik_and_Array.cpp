...
ll findans(ll in, ll flag , vector<ll>&v,vector<vector<ll>>&dp){
    ...
    if(flag == 1){
        if(abs(v[in]) % 2 != 0){
            ans = max((ll)0 ,v[in] + findans(in + 1, 2, v, dp));
        }
        else return 0;
    }
    if(flag == 2){
        if(abs(v[in]) % 2 == 0){
            ans = max((ll)0,v[in] + findans(in + 1, 1, v, dp));
        }
        else return 0;
    }
    if(flag == 0){
        ans = max(v[in] + findans(in + 1,(abs(v[in]) % 2) + 1 , v, dp), findans(in + 1, flag, v,dp));
    }
