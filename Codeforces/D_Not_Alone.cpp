...
ll findans(ll in,ll n,vector<ll>&v,vector<ll>&dp){
    if(in < 0) return 0;
    if(in == 0) return LLONG_MAX;
    if(in == 1)  return abs(v[in] - v[in - 1]);
    ...
    if(in - 1 >= 0){
        ll temp = findans(in - 2, n, v, dp);
        ans1 = (temp != LLONG_MAX ? abs(v[in] - v[in - 1]) + temp : LLONG_MAX);
    }
    if(in - 2 >= 0){
        vector<ll>temp = {v[in],v[in - 1],v[in - 2]};
        sort(all(temp));
        ll midean = temp[1];

        ll temp1 = findans(in - 3,n,v,dp);
        ans2 = (temp1 != LLONG_MAX ? abs(v[in] - midean) + abs(v[in - 1] - midean) + abs(v[in - 2] - midean) + temp1 : LLONG_MAX);
    }

    return dp[in] = min(ans1,ans2);
