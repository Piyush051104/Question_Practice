...
ll finadans(ll in,ll num,ll n ,ll m ,set<ll>&st,vector<pair<ll,char>>&v,vector<vector<ll>>&dp){
    if(in == m) {
    ...
    pair<ll,char> temp = v[in];

    ll dis = temp.first % n;
    ll ans = 0;
    if(temp.second == '?'){
        ll next = num + dis;
        if(next <= n) ans += finadans(in + 1,next,n,m,st,v,dp);
        else{
            ll extra = next - n;
            ans += finadans(in + 1,extra,n,m,st,v,dp);
        }
        next = num - dis;
        if(next >= 1) ans += finadans(in + 1,next,n,m,st,v,dp);
        else {
            ll extra = abs(next) + 1;
            ll togo = n - extra + 1;
            ans += finadans(in + 1,togo,n,m,st,v,dp);
        }
    }
...
void solve(){
    ...
    ll ans = finadans(0,x,n,m,st,v,dp);
    cout << st.size() << endl;
    for(auto &i:st){
        cout << i << " ";
    }
