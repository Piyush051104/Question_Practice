...
vector<ll>ten;
ll solve(vector<ll>&store){
    ll n;cin >> n;
    ll in = lower_bound(all(store),n) - store.begin();
    ll ans = 0;
    ll prev = ((store[in - 1])/9) * 45;
    ans += prev;
    ll rem = n - store[in - 1];
    ll prevnum = ten[in - 1] - 1;
    ll qu = rem ;
    while(qu > 0){
        string s = to_string(++prevnum);
        ll siz = s.length();
        for(ll i = 0 ;i < min(siz,qu);i++){
            ans += s[i] - '0';
