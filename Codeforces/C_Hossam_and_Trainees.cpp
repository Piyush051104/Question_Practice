...
vector<ll> get_factors(ll x, const vector<ll>& primes) {
    vector<ll> res;
...
bool solve(vector<ll>&primes){
    ...
    map<ll,ll>mp;
    for(ll i = 0 ;i < n;i++){
        vector<ll> factors = get_factors(v[i], primes);
        for(auto &j:factors){
            if(j != 1) mp[j]++;
            if(mp[j] > 1) return true;
        }
    }
    return false;
...
int main(){
    ...
    ll t; cin >> t;
    ll MAX = 1e9;
    vector<ll> primes = get_primes(sqrt(MAX) + 1);
 
    while(t--){
        bool ans = solve(primes);
