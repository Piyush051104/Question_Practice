...
long long find(long long sum, long long n) {
    long long nc2 = (n % MOD) * ((n - 1) % MOD) % MOD;
    nc2 = nc2 * power(2, MOD - 2) % MOD;   
    
    long long inv_nc2 = power(nc2, MOD - 2);
    
    long long ans = (1 + sum % MOD * inv_nc2 % MOD) % MOD;
    return ans;
}

void dfs(ll node, ll par, const vector<vector<ll>>& graph, ll n, ll &sum, vector<ll> &si) {
    si[node] = 1;
    for (ll child : graph[node]) {
        if (child == par) continue;
        dfs(child, node, graph, n, sum, si);
        si[node] += si[child];
        
        sum += si[child] * (n - si[child]);
    }
}
ll solve(){
    ll n,k;cin >> n >> k;
