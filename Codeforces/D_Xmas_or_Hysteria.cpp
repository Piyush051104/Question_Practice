...
void solve(){
    ...
    if(m > 0){
        ...
        ll in2 = 0;
        while(cnt != m - 1){
            ans.push_back({temp[in1].second,temp[in2].second});
            in1--;
            in2++;
            cnt++;
        }
        for(ll i = in2 ;i < in1;i++){
            ans.push_back({temp[i + 1].second,temp[i].second});
        }
        cout << ans.size() << endl;
        ...
        if(sum < temp[n - 1].first){
            cout << -1 << endl;
            return;
        }
        ll hel = temp[n - 1].first;
        for(ll i = 0 ;i < n - 1;i++){
            if(temp[n - 2].first < hel){
                ans.push_back({temp[i].second,temp[n - 1].second});
                hel -= temp[i].first;
            }
            else{
                ans.push_back({temp[i+1].second,temp[i].second});
