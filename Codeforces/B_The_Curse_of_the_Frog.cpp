...
ll solve(ll &cnt){
    ...
    vector<ll>temp1;
    for(auto &i:v){
        sum += ((i.first  - 1) * i.second.first);
        temp1.push_back((i.first  - 1) * i.second.first);
    }
    ...
    for(auto &i:v){
        ll add = (i.first * i.second.first) - i.second.second;
        if(add > 0){
            temp.push_back({add,{i.second.first * (i.first - 1) ,{i.second.second,i.second.first}}});
        }
    }
    ...
    sum += temp[0].second.second.second;
    sum -= temp[0].second.second.first;


    //cout << sum << " ";
    ll rem = x - sum;
    rem -= temp[0].second.first;
    ll ans = 1;

    if (rem <= 0 ) return ans;
    //cout << rem << " ";
    ans += (rem/temp[0].first);
    if(rem % temp[0].first != 0){
        ans++;
    }
    return ans;
