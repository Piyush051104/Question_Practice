...
void solve(){
    ...
    for(ll i = 0 ;i < n;i++){
        ll val = temp[i].first;
        ll valinde = *mp[val].rbegin();
        ll num = -1;
        ll index = n;
        //cout << val << " ";
        for(ll j = 1; j <= 3;j++){
            ll xorval = val ^ (val + j); 
            if(mp[val + j].size() > 0 && xorval < 4){
                ll in = *mp[val + j].begin();
                if(in < valinde){
                    if(index > in){
                        index = in;
                        num = val + j;
        ...
        //cout << valinde << " " << index << endl;
        // cout << num << endl;
        if(num != -1){
            mp[num].erase(index);
            swap(v[valinde],v[index]);
            mp[num].insert(valinde);

            mp[val].erase(valinde);
            mp[val].insert(index);
        }
        // for(ll i = 0 ;i < n;i++){
        //     cout << v[i] << " ";
