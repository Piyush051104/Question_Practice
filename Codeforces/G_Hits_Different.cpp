...
Tree* createtree(Tree* curr,ll lvl,ll par,bool flag,Tree* st){
    if(lvl == 6){
        return NULL;
    }
    if(par == -1){
        ...
        st = head;
        head->parent = NULL;
        head->left = createtree(head,lvl + 1,1,true,st);
        head->right = createtree(head,lvl + 1,1,false,st);
        return head;
    }
    else{
        if(flag){
            Tree* temp = new Tree(par + lvl);
            temp -> parent = curr;
            temp->left = createtree(temp,lvl + 1,par + lvl,true,st);
            temp->right = createtree(temp,lvl + 1,par + lvl,false,st);
            return temp;
        }
        else{
            Tree* temp = new Tree(par + lvl + 1);
            temp -> parent = curr;
            temp->left = createtree(temp,lvl + 1,par + lvl + 1,true,st);
            temp->right = createtree(temp,lvl + 1,par + lvl + 1,false,st);
             return temp;
        }
...
void levelOrderSingleLine(Tree* root) {
    if (!root) return;
    queue<Tree*> q;
    q.push(root);
    bool first = true;
...
int main(){
    ...
    Tree* head;
    Tree* ans = createtree(NULL,0,-1,true,head);

    levelOrderSingleLine(ans);
    // ll t; cin >> t;
    // while(t--){
    //     ll ans = solve();
    //     cout << ans << endl;
