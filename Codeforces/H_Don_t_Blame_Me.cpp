...
ll find(ll val,ll in,ll n,ll k,vector<ll>&v,vector<vector<ll>>&dp){
    if(in >= n){
        ll cnt = 0;
        ll num = val;
        if(num < 0) return 0;
    ...
    if(dp[in][val + 1] != -1) return dp[in][val + 1];

    ll ans = 0;
    if(val == -1){
        ans = (ans +  find(-1,in + 1,n,k,v,dp)) % MOD;
        ans = (ans +  find(v[in],in + 1,n,k,v,dp)) % MOD;
       
    }

    else{
        ll newval = val & v[in];
        ans = (ans +  find(val,in + 1,n,k,v,dp)) % MOD;
        ans = (ans +  find(newval,in + 1,n,k,v,dp)) % MOD;
        
    }

    return dp[in][val + 1] = ans % MOD;
}
ll solve(){
    ll n,k;cin >> n >> k;
    vector<ll>v(n);
    ...
    vector<vector<ll>>dp(n + 1,vector<ll>(66,-1));

    ll ans = find(-1,0,n,k,v,dp);
    return ans;
}
int main(){
