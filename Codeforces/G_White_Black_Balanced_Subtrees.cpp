...
void dfs(ll node,ll parent,vector<vector<ll>>&graph,string &s,vector<pair<ll,ll>>&color){
 
    for(auto child:graph[node]){
        if(child != parent){
           
            dfs(child,node,graph,s,color);
...
ll solve(){
    ...
    vector<pair<ll,ll>>color(n + 1,{0,0});
    dfs(1,-1,graph,s,color);
    ll ans = 0;
    for(ll i = 1 ;i <= n;i++){
        //cout << color[i].first << " " << color[i].second << endl;
        if(color[i].first == color[i].second) ans++;
    }
    return ans;
