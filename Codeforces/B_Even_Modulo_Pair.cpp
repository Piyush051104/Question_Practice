...
bool getPair(const vector<long long>& arr, long long& p, long long& q) {
    int n = arr.size();
    if (n < 2) return false;
    for (int i = 0; i + 1 < n; i++) {
        long long rem = arr[i + 1] % arr[i];
        if (rem % 2 == 0) {
            p = arr[i];
            q = arr[i + 1];
            return true;
...
void solve(){
    ...
    loop(i,n){
        cin >> v[i];
        if(v[i] % 2 == 0) temp.push_back(v[i]);
        if(v[i] % 2 != 0) temp1.push_back(v[i]);
        if(v[i] == 1) flag = true;
    }
    if(temp.size() > 1){
        ...
        long long a, b;
        if (getPair(temp1, a, b)) cout << a << " " << b << "\n";
        else cout << "-1\n";
        return;
    }
    else{
        ll even = temp[0];
        auto pos = lower_bound(temp1.begin(), temp1.end(), even);
        int cnt = pos - temp1.begin();
        bool ok = false;
