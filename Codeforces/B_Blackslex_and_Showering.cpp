...
ll solve(){
    ...
    for(ll i = 1 ;i <= n;i++){
        ...
        if(i == 1){
            ...
            mini = min(mini,ans - tosub + toadd);

        }
        else if(i == n){
            tosub = abs(v[i] - v[i - 1]);
            mini = min(mini,ans - tosub);
        }
        else{

            tosub = abs(v[i] - v[i + 1]) + abs(v[i] - v[i - 1]);
            toadd = abs(v[i + 1] - v[i - 1]);
            mini = min(mini,ans + toadd - tosub);
        }
