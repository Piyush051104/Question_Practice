...
void dfs(ll node,ll parent,ll n,ll &st,ll val,vector<vector<ll>>&graph,vector<ll>&ans){
    if(st - val == 2) st += 2;
    if((st - val) % 2 != 0 && (st -val) != 1) st++;
    ans[node] = st;
    for(auto child:graph[node]){
        if(child!=parent){
            st++;
            dfs(child,node,n,st,ans[node],graph,ans);
        }
    }
    return;
}
