...
ll findans(ll i,ll j,vector<ll>&v1,vector<ll>&v2,vector<vector<ll>>&dp){
    ...
    if(v1[i] < v1[j]){
        ans1 = v2[i] + findans(i + 1,j,v1,v2,dp);
    }
    else if(v1[i] == v1[j]) ans1 = findans(i + 1,j,v1,v2,dp);

    else{
        ans1 = findans(i + 1,i,v1,v2,dp);
        ans2 = v2[i] + findans(i + 1,j,v1,v2,dp);
    }
...
ll solve(){
    ...
    loop1(1,n + 1) cin >> v1[i];
    loop1(1,n + 1) cin >> v2[i];
    v1[0] = LLONG_MIN;
    v2[0] = 0;
    ...
    for(ll i = n ; i >= 1 ; --i){
        for(ll j = 0 ; j <= n; ++j){
            if(v1[i]  < v1[j]) curr[j] = v2[i] + prev[j];
            else if(v1[i] == v1[j]) curr[j] = prev[j];
            else{
                ll a = prev[i]; 
                ll b = v2[i] + prev[j];
                curr[j] = min(a,b);
