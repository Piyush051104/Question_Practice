...
ll dfs(ll node,ll par,ll de,vector<vector<ll>>&graph,vector<vector<ll>>&depth,vector<vector<ll>>&maxdep){
    depth[de].push_back(node);
    ll maxi = de;
    for(auto &child : graph[node]){
        if(child == par) continue;
        maxi = max(maxi,dfs(child,node,de + 1,graph,depth,maxdep));
    }

    maxdep[maxi].push_back(node);
...
ll solve(){
    ...
    vector<vector<ll>>maxdep(n + 1);
    vector<vector<ll>>depth(n + 1);
    dfs(1,-1,0,graph,depth,maxdep);
    vector<ll>desize(n + 1);
    vector<ll>maxdesi(n + 1);
    for(ll i = 0 ;i <= n;i++){
        desize[i] = depth[i].size();
        maxdesi[i] = maxdep[i].size();
    }

    vector<ll>depsum(n + 1);
    vector<ll>maxdesum(n + 1);
    depsum[0] = desize[0];
    maxdesum[0] = maxdesi[0];

    for(ll i = 1 ;i <= n;i++){
        depsum[i] = depsum[i - 1] + desize[i];
        maxdesum[i] = maxdesum[i - 1] + maxdesi[i];
    }

    ll ans = n;
    for(ll i = 0 ;i < n;i++){
        ll sum1 = 0;
        if(i - 1 >= 0) sum1 = maxdesum[i - 1];
        ll sum2 = depsum[n] - depsum[i];
