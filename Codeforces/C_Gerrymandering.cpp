...
ll solve(){
    ...
    vector<vector<ll>> vot(2, vector<ll>(n + 5, 0));
    for(int i = 0; i < n; i++){
        if(s1[i] == 'A') vot[0][i + 1] = 1;
        if(s2[i] == 'A') vot[1][i + 1] = 1;
    }
    ...
    for(ll k = 0; k <= n - 1; k++){
        for(ll i = 0; i < 3; i++){
            ...
            if(i == 0){
                if(k + 3 <= n){
                    ll vt =
                        (vot[0][k+1] + vot[0][k+2] + vot[0][k+3]) / 2 +
                        (vot[1][k+1] + vot[1][k+2] + vot[1][k+3]) / 2;
                    dp[k+3][0] = max(dp[k+3][0], val + vt);
                }
                if(k + 1 <= n){
                    ll vt =
                        (vot[1][k+1] + vot[0][k+1] + vot[0][k+2]) / 2;
                    dp[k+1][1] = max(dp[k+1][1], val + vt);

                    vt =
                        (vot[0][k+1] + vot[1][k+1] + vot[1][k+2]) / 2;
            ...
            if(i == 1){
                if(k + 3 <= n){
                    ll vt =
                        (vot[0][k+2] + vot[0][k+3] + vot[0][k+4]) / 2 +
                        (vot[1][k+1] + vot[1][k+2] + vot[1][k+3]) / 2;
                    dp[k+3][1] = max(dp[k+3][1], val + vt);
                }
                if(k + 2 <= n){
                    ll vt =
                        (vot[1][k+1] + vot[1][k+2] + vot[0][k+2]) / 2;
            ...
            if(i == 2){
                if(k + 3 <= n){
                    ll vt =
                        (vot[1][k+2] + vot[1][k+3] + vot[1][k+4]) / 2 +
                        (vot[0][k+1] + vot[0][k+2] + vot[0][k+3]) / 2;
                    dp[k+3][2] = max(dp[k+3][2], val + vt);
                }
                if(k + 2 <= n){
                    ll vt =
                        (vot[0][k+1] + vot[0][k+2] + vot[1][k+2]) / 2;
                    dp[k+2][0] = max(dp[k+2][0], val + vt);
                }
            }
        }
