...
ll solve(){
    ...
    for(ll i = 2; i<= maxi;i++){
        ll totalcost = 0;
        for(ll j = 1 ;j <= (maxi + i - 1)/i ;j++){
            ll maxinum = min(j * i,maxi);
            ll minnum = (j * i - i) + 1;
            ll cnt = prefix[maxinum] - prefix[minnum - 1];
            totalcost += (j * cnt);
            ll freq = prefix[j] - prefix[j - 1];
            if(cnt > freq){
                totalcost -= (cnt - freq) * y;
            }
        }
