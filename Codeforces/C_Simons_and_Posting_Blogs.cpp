...
void correct(vector<vector<ll>>& v){
    for (auto &vec : v){
        if (vec.empty()) continue;
        unordered_set<ll> seen_local;
        vector<ll> tmp;
        tmp.reserve(vec.size());
        for (ll x : vec){
            if (!seen_local.count(x)){
                tmp.push_back(x);
                seen_local.insert(x);
            }
        }
...
void solve(){
    ...
    vector<bool>flag(1e6 + 1,false);
    for(ll i = 0 ;i < n;i++){
        if(v.empty()) break;
        sort(all(v));
        vector<ll>temp = v[0];
        for(ll j = 0 ; j < temp.size();j++){
            if(flag[temp[j]] == false) topri.push_back(temp[j]);
            flag[temp[j]] = true;
        }
        vector<vector<ll>>toco;
        for(ll j = 1 ;j < v.size();j++){
            vector<ll>anote;
            for(ll k = 0 ; k < v[j].size();k++){
                if(flag[v[j][k]] == false){
                    anote.push_back(v[j][k]);
                }
            }
            if(anote.size() > 0) toco.push_back(anote);
        }
        if(toco.size() == 0) break;
        v = toco;
        correct(v);
    }
    for(ll i = 0 ;i < topri.size();i++){
        cout << topri[i] << " ";
    }
    cout << endl;
