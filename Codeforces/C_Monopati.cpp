...
ll solve(){
    ...
    vector<pair<ll,ll>> temp1(n);
    vector<pair<ll,ll>> temp2(n);

    ll mini = v[0][0], maxi = v[0][0];
    loop(i,n){
        mini = min(mini, v[0][i]);
        maxi = max(maxi, v[0][i]);
        temp1[i] = {mini, maxi};
    ...
    for (int i = 0; i < n; ++i) {
        ll mn1 = min(temp1[i].first, temp2[i].first);
        ll mx1 = max(temp1[i].second, temp2[i].second);
        if (mn1 > mx1) continue;
