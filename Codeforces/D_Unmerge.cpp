...
bool findans(ll in,ll sum,vector<ll>&temp,vector<vector<ll>>&dp){
    if(in < 0) return false;
    if(sum == 0) return true;
    if(sum < 0) return false;
    if(dp[in][sum] != -1) return dp[in][sum];

    bool ans1 = findans(in - 1,sum,temp,dp);
    bool ans2 = findans(in - 1,sum - temp[in],temp,dp);

    return dp[in][sum] = ans1 || ans2;
...
bool solve(){
    ...
    ll si = temp.size();
    vector<vector<ll>>dp(si + 1,vector<ll>(n + 1,-1));
    bool ans = findans(si - 1,n,temp,dp);
    return ans;
