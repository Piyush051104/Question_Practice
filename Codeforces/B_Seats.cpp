...
ll solve(){
    ...
    int dp[2][2], ndp[2][2];
    for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) dp[a][b] = INT_MAX;

    int must1 = s[0] - '0';
    for (int b = 0; b <= 1; b++) if (b >= must1) dp[0][b] = b;

    for (int i = 1; i <= n - 1; i++) {
        for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) ndp[a][b] = INT_MAX;
        ...
        for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) dp[a][b] = ndp[a][b];
    }
