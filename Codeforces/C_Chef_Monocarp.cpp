...
ll find(ll in,ll time,ll n,vector<ll>&v,vector<vector<ll>>&dp){
    if(in >= n) return 0;
    if(time > 2 * n) return LLONG_MAX;

    if(dp[in][time] != -1) return dp[in][time];

    ll ans1 = LLONG_MAX;
    ll val1 = find(in + 1,time + 1,n,v,dp);
    ll ans2 = LLONG_MAX;
    ll val2 = find(in,time + 1,n,v,dp);

    if(val1 != LLONG_MAX){
        ans1 = abs(time - v[in]) + val1;
    }
    if(val2 != LLONG_MAX){
        ans2 = val2;
    }

    return dp[in][time] = min(ans1,ans2);
}
ll solve(){
    ll n;cin >> n;
    vector<ll>v(n);
    ...
    vector<vector<ll>>dp(n + 1,vector<ll>(3 * n + 1,-1));
    sort(all(v));

    ll ans = find(0,1,n,v,dp);
    return ans;
