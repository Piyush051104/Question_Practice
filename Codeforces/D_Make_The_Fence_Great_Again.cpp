...
ll find(ll in,ll ch,ll n,vector<pair<ll,ll>>&v,vector<vector<ll>>&dp){
    if(in >= n) return 0;

    if(dp[in][ch] != -1) return dp[in][ch];

    ll ans = LLONG_MAX;

    for(ll i = 0 ;i <= 2;i++){
        if(in == 0){
            ans = min(ans,i * v[in].second + find(in + 1,i,n,v,dp));
        }
        else{
            if(v[in - 1].first + ch != v[in].first + i){
                ans = min(ans,i * v[in].second + find(in  + 1,i,n,v,dp));
            }
        }
    }
