...
ll dfs(ll node,ll par,vector<ll>&count,vector<vector<ll>>&graph,vector<ll>&parent,ll n,ll k,ll &ans,vector<ll>&dp){
    ...
    for(auto &child:graph[node]){
        if(child != par){
            parent[child] = node;
            ischild = false;
            ll a = dfs(child,node,count,graph,parent,n,k,ans,dp);
            count[node] += a;
            if(n - a >= k){
                ans += a;
                dp[node] += a;
...
ll solve(){
    ...
    vector<ll>dp(n + 1,0);
    ll ans = 0;
    dfs(1,-1,count,graph,parent,n,k,ans,dp);
    // for(ll i = 1 ;i <= n;i++){
    //     cout << dp[i] << " ";
    // }
    //cout << endl;
    for(ll i = 2 ;i <= n;i++){
        if(count[i] >= k) ans += (n - count[i]); 
