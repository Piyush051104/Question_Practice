...
ll solve(){
    ...
    for(ll i = 0 ; i < k ;i++){
        if(abs(v[i].first - v[b - 1].first) + abs(v[i].second - v[b - 1].second) < dis1){
            dis1 = abs(v[i].first - v[b - 1].first) + abs(v[i].second - v[b - 1].second);
            x1 = v[i].first;
            y1 = v[i].second;
        }
    }
    ...
    ll dis2 = LLONG_MAX;
    for(ll i = 0 ; i < k ;i++){
        if(abs(v[i].first - v[a - 1].first) + abs(v[i].second - v[a - 1].second) < dis2){
            dis2 = abs(v[i].first - v[a - 1].first) + abs(v[i].second - v[a - 1].second);
            x2 = v[i].first;
            y2 = v[i].second;
        }
    }
    //cout << dis2 << " ";
    ll dis  = abs(v[b - 1].first - v[a - 1].first) + abs(v[b - 1].second - v[a - 1].second);
    if(k == 0) return dis;
    return min(dis , dis1 + dis2);
}
int main(){
