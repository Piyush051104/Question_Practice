...
ll find(ll i1,ll i2,string &a,string &b,string &c,vector<vector<ll>>&dp){
    ...
    if(i1 != a.length() && a[i1] != c[pos]){
        ll val1 = find(i1 + 1,i2,a,b,c,dp);
        if(val1 != LLONG_MAX) ans = min(ans,1 + val1);
    }
    if(i1 != a.length() && a[i1] == c[pos]){
        ll val1 = find(i1 + 1,i2,a,b,c,dp);
        if(val1 != LLONG_MAX) ans = min(ans,val1);
    }
    if(i2 != b.length() && b[i2] != c[pos]){
        ll val1 = find(i1,i2 + 1,a,b,c,dp);
        if(val1 != LLONG_MAX) ans = min(ans,1 + val1);
    }
    if(i2 != b.length() && b[i2] == c[pos]){
        ll val1 = find(i1,i2 + 1,a,b,c,dp);
