...
bool solve(){
    ...
    if(mini & 1){
        ...
        for(ll i = 0 ;i < n;i++){
            if(v[i] % 2 == 0) t1.push_back(v[i]);
        }
    }
    if(maxi & 1){
        for(ll i = 0 ;i < n;i++){
            if(v[i] & 1) t3.push_back(v[i]);
        }
    }
    else{
        for(ll i = 0 ;i < n;i++){
            if(v[i] % 2 == 0) t3.push_back(v[i]);
        }
    }
    vector<ll>t2 = t1;
    ...
    sort(all(t2));
    sort(all(t4));
    if(t2 == t1 && t3 == t4 && abs(maxi - mini) % 2  == 0) return true;
    if(abs(maxi - mini) % 2  != 0) return true;
    return false;
