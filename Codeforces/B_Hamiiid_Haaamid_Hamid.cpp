...
//     ll n,in; cin >> n >> in;
//     string s; cin >> s;
//     if(in == 1 || in == n) return 1;
//     ll in1 = -1 ,in2 = n;
//     for(ll i = 0 ;i < n ;i++){
//         if(s[i] == '#'){
//             if(i < in -1) in1 = i;
//             else  if(i >= in - 1 && in2==n) in2 = i;
//         }

//     }
   
//     if(in1==-1 && in2 ==n) return 1;
//     ll ans1 = min(in,n - in2 + 1);
//     ll ans2= min(in1 + 2,n - in + 1);
...
ll solve(){
    ...
    in++;
    ll w2 = n - in + 1;
    ll ans2 = min(w2,in1 + 1);
    ll w1 = in;
    ll ans1 = min(w1,n - in2 + 2);
    return max(ans1,ans2);   
}
int main(){
