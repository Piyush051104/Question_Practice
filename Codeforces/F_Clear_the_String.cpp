...
ll findans(ll l,ll r,string &s,vector<vector<ll>>&dp){
    if(l > r) return 0;
    if(l == r) return 1;
    if(dp[l][r] != -1) return dp[l][r];

    ll ans1 = 1 + findans(l + 1, r, s, dp);

    for(ll i = l + 1; i <= r; i++){
        if(s[i] == s[l]){
            ans1 = min(ans1, findans(l + 1, i - 1, s, dp) + findans(i, r, s, dp));
        }
    }
