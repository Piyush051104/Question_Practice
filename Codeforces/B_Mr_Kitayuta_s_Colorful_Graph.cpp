...
void dfs(ll node,ll source,ll parent,ll colo,vector<vector<pair<ll,ll>>>&graph,vector<bool>&vis,set<ll>&st){
    vis[node]=true;
    if(node==source){
        st.insert(colo);
        //return;
    ...
    for(auto child:graph[node]){
        if(vis[child.first]==false && (child.second==colo || colo==-1)){
            vis[child.first]=true;
            dfs(child.first,source,node,child.second,graph,vis,st);
