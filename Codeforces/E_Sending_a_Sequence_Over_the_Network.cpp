...
bool find(ll sum,ll cnt,ll in,vector<ll>&v,ll n){
    if(in >= n) return false;

    bool ans1 = false,ans2 = false;
    if(sum == n - cnt) return true;
    if(sum + v[in] == n - cnt - 1) return true;
    else{
        ans1 = find(sum + v[in],cnt + 1,in + 1,v,n);
        ans2 = find(sum,cnt,in + 1,v,n);
    }
