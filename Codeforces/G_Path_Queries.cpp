...
void solve(){
    ...
    loop(i,q){
        ...
        while(st < n - 1 && v[st].first <= weight){
            ll up = dsu.findpar(v[st].second.first);
            ll vp = dsu.findpar(v[st].second.second);
            ll su = dsu.size[up];
            ll sv = dsu.size[vp];
            if(up != vp){
                ll x = (su*(su - 1))/2;
                ...
                ll val = (add *(add - 1))/2;
                cnt = cnt - x - y + val;
                dsu.unionbysize(v[st].second.first,v[st].second.second);
            }
