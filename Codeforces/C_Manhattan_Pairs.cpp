...
bool comp1(pair<ll,pair<ll,ll>>p1,pair<ll,pair<ll,ll>>p2){
...
void solve(){
    ...
    sort(all(v),comp);
    vector<pair<ll,pair<ll,ll>>>temp1,temp2;
    for(ll i=0;i<n/2;i++){
        temp1.push_back(v[i]);
    ...
    sort(all(temp2),comp1);

    for(ll i=0;i<n/2;i++){
        cout<<temp1[i].second.second<<" "<<temp2[temp2.size()-1-i].second.second<<endl;
    }
    return;
}
int main(){
