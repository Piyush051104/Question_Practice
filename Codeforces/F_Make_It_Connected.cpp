...
ll findpar(ll node,vector<ll>&parent){
    if(parent[node] == node) return node;
    else return findpar(parent[node],parent);
}
ll solve(){
    ll n,k;cin >> n >> k;
    ...
    sort(all(v));
    vector<pair<ll,pair<ll,ll>>>weight;
    loop(i,k){
        ll a,b,c;cin >> a >> b >> c;
        weight.push_back({c,{a,b}});
    }
    for(ll i = 1;i < n;i++){
        weight.push_back({v[0].first + v[i].first,{v[0].second,v[i].second}});
    }
    sort(all(weight));
    ...
    for(ll i = 0 ;i < weight.size() ;i++){
        ll up = findpar(weight[i].second.first,parent);
        ll vp = findpar(weight[i].second.second,parent);
        if(up == vp) continue;
        else{
            if(size[up] > size[vp]){
                parent[vp] = up;
