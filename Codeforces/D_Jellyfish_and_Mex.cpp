...
ll finans(ll num,ll mex,vector<vector<ll>>&dp,vector<pair<ll,ll>>&temp){
    if(num == 0){
        dp[0][mex] = (temp[0].second - 1) * mex;
        return dp[0][mex];
    }

    if(dp[num][mex] != -1) return dp[num][mex];

    ll ans1 = finans(num - 1,mex,dp,temp);
    ll ans2 = (temp[num].second - 1) * mex + num + finans(num - 1,num,dp,temp);

    return dp[num][mex] = min(ans1,ans2);
}
ll solve(){
    ll n;cin >> n;
    vector<ll>v(n);
