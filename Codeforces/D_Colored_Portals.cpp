...
void solve(){
    ...
    loop(i,q){
        ...
        if(a > b) swap(a,b);
        //cout << a << " " << b;
        if(temp[a][0] == temp[b][0] || temp[a][0] == temp[b][1] || temp[a][1] == temp[b][0] || temp[a][1] == temp[b][1]){
            cout << b - a << endl;
            continue;
        }
        else{
            ...
            if(flag == false){
                ...
                for(ll j = 0 ;j < 4;j++){
                    string s = str[in][j];
                    if(v[mp[s]].size() > 0){
                        ll in1 = lower_bound(all(v[mp[s]]),a) - v[mp[s]].begin();

                        if(in1 < v[mp[s]].size() && v[mp[s]][in1] < b){
                            ans = b - a;
                            break;;
                        }
                        if(in1 != 0){
                            ll r = v[mp[s]][in1 - 1];
                            ans = min(ans,abs(r - a) + abs(r - b));
                        }
                        else{
                            ll r = v[mp[s]][in1];
                            ans = min(ans,abs(r - a) + abs(r - b));
