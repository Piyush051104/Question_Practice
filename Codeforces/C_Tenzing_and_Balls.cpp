...
ll find(ll in,map<ll,vector<ll>>&mp,vector<ll>&v,vector<ll>&dp,ll n){
    if(in >= n) return 0;
    ...
    ll index = upper_bound(mp[v[in]].begin(),mp[v[in]].end(),in) - mp[v[in]].begin();

    if(index != mp[v[in]].size()){
        for(ll i = index ; i < mp[v[in]].size();i++){
            ans = max(ans,mp[v[in]][i] - in + 1 + find(mp[v[in]][i] + 1,mp,v,dp,n));
        }
    }
    ans = max(ans,find(in + 1,mp,v,dp,n));
...
ll solve(){
    ...
    for(ll i = n - 1 ;i >= 0;i--){
        ...
        if(best[v[i]] != LLONG_MIN){
            dp[i] = max(dp[i],best[v[i]] - i);
        }

        best[v[i]] = max(best[v[i]], i + 1 + dp[i + 1]);
    }
